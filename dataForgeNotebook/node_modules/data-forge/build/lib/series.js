...
var Series = /** @class */ (function () {
    ...
    */
     ...
     * </pre>
     */
    Series.prototype.select = function (transformationFn) {
        if (!utils_1.isFunction(transformationFn))
            throw new Error("Expected 'transformationFn' parameter to 'Series.select' to be a function.");
        return this.map(transformationFn);
    };
    /**
     * Transforms an input series, generating a new series.
     * The transfomration function is called for each value in the input series and produces a new value that is output to the generated series.
     ...
     * </pre>
     */
    Series.prototype.map = function (transformationFn) {
        var _this = this;
        if (!utils_1.isFunction(transformationFn))
            throw new Error("Expected 'transformationFn' parameter to 'Series.map' to be a function.");
        return new Series(function () { return ({
            values: new select_iterable_1.SelectIterable(_this.getContent().values, transformationFn),
     ...
     * </pre>
     */
    Series.prototype.selectMany = function (transformationFn) {
        if (!utils_1.isFunction(transformationFn))
            throw new Error("Expected 'transformationFn' parameter to 'Series.selectMany' to be a function.");
        return this.flatMap(transformationFn);
    };
    /**
     * Transforms and flattens an input series, generating a new series.
     * The transfomration function is called for each value in the input series and produces an array that is flattened into the generated series.
     ...
     * </pre>
     */
    Series.prototype.flatMap = function (transformationFn) {
        var _this = this;
        if (!utils_1.isFunction(transformationFn))
            throw new Error("Expected 'transformationFn' parameter to 'Series.flatMap' to be a function.");
        return new Series(function () { return ({
            pairs: new select_many_iterable_1.SelectManyIterable(_this.getContent().pairs, function (pair, index) {
                var outputPairs = [];
                try {
                    for (var _a = __values(transformationFn(pair[1], index)), _b = _a.next(); !_b.done; _b = _a.next()) {
